...
$shouldBeRemovedAsTeamOwnerArray = @()
...
foreach ($team in $teams) {
    Write-Host "***********************************************************"
    Write-Host "Checking user team ownership for team ""$($team.DisplayName)"""
    ...
    $shouldBeRemovedAsTeamOwnerArray += $shouldBeRemovedAsTeamOwner
    #endregion Handle user Team ownership
}

Write-Host
Write-Host "Waiting for 60 seconds before processing the teams..." -ForegroundColor Yellow
...
foreach ($team in $teams) {
    ...
    if (![string]::IsNullOrEmpty($spSiteUrl)) {
        ...
        if ($shouldBeRemovedAsTeamOwnerArray[$i++] -or $ForceRemoveUserAsOwner) {
